...
#include <rarexsec/data/EventProcessor.h>  // was IEventProcessor.h

namespace analysis {

class TruthClassifier : public EventProcessor {
public:
  explicit TruthClassifier() = default;
...
private:
  ROOT::RDF::RNode processNonMc(ROOT::RDF::RNode df, Origin origin) const {
    ...
    auto incl_df = mode_df.Define("incl_channel", [c = origin]() {
      if (c == Origin::kData)     return 0; // Data
      if (c == Origin::kExternal) return 1; // External
      if (c == Origin::kDirt)     return 2; // Dirt
    ...
    auto chan_df = excl_alias_df.Define("channel_def", [c = origin]() {
      if (c == Origin::kData) return 0; // Data
      if (c == Origin::kExternal || c == Origin::kDirt) return 1; // External
      return 99; // Other / Unknown
    });
  ...
  ROOT::RDF::RNode assignInclusiveChannels(ROOT::RDF::RNode df) const {
    // NOTE: npi (pions) comes from mc_n_pion; np (protons) from mc_n_proton.
    auto incl_chan_df =
      df.Define("incl_channel",
        [](bool fv, int nu, int ccnc, int s, int npi, int np) {
          if (!fv)            return 98; // out_fv
          if (ccnc == 1)      return 31; // nc
          if (std::abs(nu) == 12 && ccnc == 0) return 30; // nue_cc
          if (std::abs(nu) == 14 && ccnc == 0) {
            if (s == 1)       return 10; // numu_cc_1s
            if (s > 1)        return 11; // numu_cc_ms
            if (np >= 1 && npi == 0) return 20; // numu_cc_np0pi
            if (np == 0 && npi >= 1) return 21; // numu_cc_0pnpi
            if (np >= 1 && npi >= 1) return 22; // numu_cc_npnpi
  ...
  ROOT::RDF::RNode assignExclusiveChannels(ROOT::RDF::RNode df) const {
    auto excl_chan_df =
      df.Define("excl_channel",
        [](bool fv, int nu, int ccnc, int s,
           int kp, int km, int k0, int lam, int sp, int s0, int sm) {
          if (!fv)                 return 98; // out_fv
          if (ccnc == 1)           return 31; // nc
          if (std::abs(nu) == 12 && ccnc == 0) return 30; // nue_cc
          if (std::abs(nu) == 14 && ccnc == 0) {
            if (s == 0)                           return 32; // numu_cc_other
            if ((kp == 1 || km == 1) && s == 1)  return 50; // numu_cc_kpm
            if (k0 == 1 && s == 1)               return 51; // numu_cc_k0
            if (lam == 1 && s == 1)              return 52; // numu_cc_lambda
            if ((sp == 1 || sm == 1) && s == 1)  return 53; // numu_cc_sigmapm
            if (lam == 1 && (kp == 1 || km == 1) && s == 2) return 54; // lambda+k±
            if ((sp == 1 || sm == 1) && k0 == 1 && s == 2)  return 55; // sigma+k0
            if ((sp == 1 || sm == 1) && (kp == 1 || km == 1) && s == 2) return 56;
            if (lam == 1 && k0 == 1 && s == 2)   return 57; // lambda+k0
            if (kp == 1 && km == 1 && s == 2)    return 58; // k± + k∓
            if (s0 == 1 && s == 1)               return 59; // sigma0
            if (s0 == 1 && kp == 1 && s == 2)    return 60; // sigma0 + k±
            return 61; // other strange
          }
          return 99; // other
        },
  ...
  ROOT::RDF::RNode assignChannelDefinitions(ROOT::RDF::RNode df) const {
    auto chan_df =
      df.Define("channel_definitions",
        [](bool fv, int nu, int ccnc, int s, int npi, int np, int npi0, int ngamma) {
          if (!fv) {
            if (nu == 0) return 1; // External
            return 2;              // Out FV
          }
          if (ccnc == 1) return 14; // nc
          if (ccnc == 0 && s > 0) {
            if (s == 1) return 15; // numu_cc_1s
            return 16;             // numu_cc_ms
          }
          if (std::abs(nu) == 12 && ccnc == 0) return 17; // nue_cc
          if (std::abs(nu) == 14 && ccnc == 0) {
            if (npi == 0 && np > 0)           return 10; // numu_cc_np0pi
            if (npi == 1 && npi0 == 0)        return 11; // numu_cc_0pnpi
            if (npi0 > 0 || ngamma >= 2)      return 12; // numu_cc_pi0gg
            if (npi > 1)                      return 13; // numu_cc_npnpi
            return 18;                                // other
          }
          return 99; // other
        },
    ...
    auto signal_df =
      chan_df.Define("is_truth_signal",
                     [](int ch) { return ch == 15 || ch == 16; },
                     {"channel_definitions"});

    auto pure_sig_df =
      signal_df.Define("pure_slice_signal",
