...
#include <rarexsec/data/EventProcessor.h>  // base class
#include <rarexsec/data/Types.h>           // Origin/Role/Variation
...
public:
  ROOT::RDF::RNode process(ROOT::RDF::RNode df, Origin origin) const override {
    ...
    // ---- Normalize column names (support legacy ones) -----------------------
    ...
    if (!node.HasColumn("reco_neutrino_vertex_sce_x") && node.HasColumn("reco_nu_vtx_sce_x"))
      node = node.Alias("reco_neutrino_vertex_sce_x", "reco_nu_vtx_sce_x");
    if (!node.HasColumn("reco_neutrino_vertex_sce_y") && node.HasColumn("reco_nu_vtx_sce_y"))
      node = node.Alias("reco_neutrino_vertex_sce_y", "reco_nu_vtx_sce_y");
    if (!node.HasColumn("reco_neutrino_vertex_sce_z") && node.HasColumn("reco_nu_vtx_sce_z"))
      node = node.Alias("reco_neutrino_vertex_sce_z", "reco_nu_vtx_sce_z");

    // ---- Basic reconstruction helpers --------------------------------------
    node = node.Define(
      "in_reco_fiducial",
    ...
    // Software trigger (handles several schema variants)
    if (origin == Origin::kMonteCarlo) {
      if (node.HasColumn("software_trigger_pre_ext")) {
        node = node.Define("software_trigger",
    ...
    // ---- NuMu-CC selection flags ------------------------------------------
    // Pre: dataset gate (for MC/Dirt) + software trigger
    node = node.Define(
      "pass_pre",
    ...
    // Muon requirement (robust if muon features were not run)
    ...
    // Backward-compatible summary gate (was Reconstruction::quality_event)
    node = node.Define("quality_event", "pass_pre && pass_flash && pass_fv && pass_topo");
