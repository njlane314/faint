...
UnionSamples rarexsec::syst::collect_union_samples(const Hub& hub,
    ...
    for (auto* e : sim) {
        ...
        case sample::origin::beam:
            s.A_beam.push_back(e);
            break;
        case sample::origin::strangeness:
            s.B_strange.push_back(e);
            break;
...
static TMatrixDSym block_cov_from_detvar_unisims_AB(const plot::Histogram1DSpec& specA, const std::vector<const Entry*>& A,
    ...
    for (const auto& t : tags) {
        ...
        for (int i = 0; i < nA; ++i)
            d[i] = HA->GetBinContent(i + 1) - H0A->GetBinContent(i + 1);
        for (int j = 0; j < nB; ++j)
            d[nA + j] = HB->GetBinContent(j + 1) - H0B->GetBinContent(j + 1);
    ...
    for (int i = 0; i < n; ++i)
        for (int j = i; j < n; ++j) {
            long double s = 0.0L;
            for (int u = 0; u < N; ++u)
                s += (long double)deltas[u][i] * (long double)deltas[u][j];
...
UnionProducts rarexsec::syst::build_union_systematics(const plot::Histogram1DSpec& spec,
    ...
    if (cfg.include_dirt)
        out.H_DIRT = make_total_mc_hist(spec, samp.dirt, "_DIRT");
    if (cfg.include_ext)
        out.H_EXT = make_total_mc_hist(spec, samp.ext, "_EXT");
    if (!samp.data.empty())
        out.H_data = make_total_mc_hist(spec, samp.data, "_data");
    ...
    if (cfg.include_dirt)
        BD_entries.insert(BD_entries.end(), samp.dirt.begin(), samp.dirt.end());
    ...
    if (cfg.use_ppfx) {
        int N = cfg.N_ppfx;
        if (N < 0)
            N = rarexsec::syst::detect_n_univ_ushort(spec, samp.A_beam, cfg.ppfx_branch, 600);
    ...
    if (cfg.use_genie) {
        int N = cfg.N_genie;
        if (N < 0)
            N = rarexsec::syst::detect_n_univ_map(spec, samp.A_beam, cfg.map_branch, cfg.genie_key, 500);
    ...
    if (cfg.use_reint) {
        int N = cfg.N_reint;
        if (N < 0)
            N = rarexsec::syst::detect_n_univ_map(spec, samp.A_beam, cfg.map_branch, cfg.reint_key, 100);
    ...
    if (cfg.use_detvar && (!cfg.detvar_pairs.empty() || !cfg.detvar_unisims.empty())) {
        ...
        if (any)
            out.C_block_sources["Detector"] = std::move(Cdet);
    }
