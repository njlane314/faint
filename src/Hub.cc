...
inline rarexsec::Slice parse_slice_opt(const json& j) {
  if (!j.contains("slice")) return rarexsec::Slice::None;
  const auto s = to_lower(j.at("slice").get<std::string>());
  if (s == "beam" || s == "beaminclusive") return rarexsec::Slice::BeamInclusive;
...
inline std::pair<rarexsec::Source, rarexsec::Slice>
parse_kind_slice(const std::string& kind, const json& s) {
  if (kind == "data") return {rarexsec::Source::Data, rarexsec::Slice::None};
  if (kind == "ext" || kind == "external") return {rarexsec::Source::Ext, rarexsec::Slice::None};
  if (kind == "mc")  return {rarexsec::Source::MC, parse_slice_opt(s)};
  if (kind == "beam")        return {rarexsec::Source::MC, rarexsec::Slice::BeamInclusive};
  if (kind == "strangeness") return {rarexsec::Source::MC, rarexsec::Slice::StrangenessInclusive};
  if (kind == "dirt")        return {rarexsec::Source::MC, rarexsec::Slice::None};
...
rarexsec::Hub::simulation_entries(const std::string& beamline,
  ...
  for (const auto& per : periods) {
    ...
    for (const auto& rec : it_p->second) {
      if (rec.source != Source::Data) out.push_back(&rec);
    }
  }
  return out;
}
