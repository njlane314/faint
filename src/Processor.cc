  ...
  if (is_mc) {
    node = node.Define(
      ...
      [opt](double w, double w_spline, double w_tune) {
        ...
        if (out < 0.0) return opt.clamp_negative_weights_to_zero ? 0.0 : out;
        return out;
      },
  ...
  if (is_mc && opt.make_truth_classification) {
    ...
    node = node.Define(
      ...
      [](bool fv, int nu, int ccnc, int s, int np, int npim, int npip, int npi0, int ngamma) {
        ...
        if (!fv) {
          if (nu == 0) return static_cast<int>(Channel::OutFV);
          return static_cast<int>(Channel::External);
        }
        if (ccnc == 1) return static_cast<int>(Channel::NC);
        if (ccnc == 0 && s > 0) {
          if (s == 1) return static_cast<int>(Channel::CCS1);
          return static_cast<int>(Channel::CCSgt1);
        }
        if (std::abs(nu) == 12 && ccnc == 0) return static_cast<int>(Channel::ECCC);
        if (std::abs(nu) == 14 && ccnc == 0) {
          if (npi == 0 && np > 0)       return static_cast<int>(Channel::MuCC0pi_ge1p);
          if (npi == 1 && npi0 == 0)    return static_cast<int>(Channel::MuCC1pi);
          if (npi0 > 0 || ngamma >= 2)  return static_cast<int>(Channel::MuCCPi0OrGamma);
          if (npi > 1)                  return static_cast<int>(Channel::MuCCNpi);
    ...
    if (opt.make_signal_flags) {
      ...
      node = node.Define(
        ...
        [opt](bool is_sig, float purity, float completeness) {
          return is_sig && purity > static_cast<float>(opt.recognised_purity_min)
                        && completeness > static_cast<float>(opt.recognised_completeness_min);
        },
    ...
    const int nonmc_channel =
      ...
      (is_data ? static_cast<int>(Channel::DataInclusive) : static_cast<int>(Channel::Unknown));
