...
void make_phase_space_combined_plots() {
    ...
    std::vector<const rarexsec::Entry*> A_beam, B_strange;
    for (auto* e : sim) {
        if (!e)
            continue;
        if (e->kind == sample::origin::beam)
            A_beam.push_back(e);
        else if (e->kind == sample::origin::strangeness)
            B_strange.push_back(e);
    }

    plot::Histogram1DSpec spec;
    spec.id = "enu";
    spec.title = "Reconstructed E_{#nu};E_{#nu}^{reco} [GeV];Events";
    spec.nbins = 20;
    spec.xmin = 0.0;
    spec.xmax = 2.0;
    spec.expr = "reco_nu_energy";
    spec.weight = "evt_weight";
    spec.sel = "";

    auto H_A = rarexsec::syst::make_total_mc_hist(spec, A_beam, "_A");
    auto H_B = rarexsec::syst::make_total_mc_hist(spec, B_strange, "_B");

    auto H_data = rarexsec::syst::make_total_mc_hist(spec, data, "_data");

    TMatrixDSym C_stat_block = rarexsec::syst::block_diag_stat(*H_A, *H_B);

    const int Nppfx = 600;
    TMatrixDSym C_ppfx_block = rarexsec::syst::block_cov_from_weight_vector_ushort_scaled(
        spec, A_beam, spec, B_strange, "weightsPPFX", Nppfx, "ppfx_cv", 1.0 / 1000.0);

    const int Ngenie = 500;
    TMatrixDSym C_genie_block = rarexsec::syst::block_cov_from_map_weight_vector(
        spec, A_beam, spec, B_strange, "weights", "All_UBGenie", Ngenie,
        "weightSplineTimesTune");

    const int Nreint = 100;
    TMatrixDSym C_reint_block = rarexsec::syst::block_cov_from_map_weight_vector(
        spec, A_beam, spec, B_strange, "weights", "reint_all", Nreint, "");

    TMatrixDSym C_pot_block = rarexsec::syst::pot_cov_block(*H_A, *H_B, 0.02);

    std::vector<const TMatrixDSym*> src_blocks = {
        &C_stat_block, &C_ppfx_block, &C_genie_block, &C_reint_block, &C_pot_block};
    TMatrixDSym C_block_total = rarexsec::syst::sum(src_blocks);

    auto H_sum = rarexsec::syst::sum_same_binning(*H_A, *H_B, "h_sum");
    TMatrixDSym C_sum_total = rarexsec::syst::sum_covariance_block_same_binning(C_block_total,
                                                                                H_A->GetNbinsX(),
                                                                                H_B->GetNbinsX());

    TMatrixDSym C_sum_stat = rarexsec::syst::sum_covariance_block_same_binning(C_stat_block, H_A->GetNbinsX(), H_B->GetNbinsX());
    TMatrixDSym C_sum_ppfx = rarexsec::syst::sum_covariance_block_same_binning(C_ppfx_block, H_A->GetNbinsX(), H_B->GetNbinsX());
    TMatrixDSym C_sum_genie = rarexsec::syst::sum_covariance_block_same_binning(C_genie_block, H_A->GetNbinsX(), H_B->GetNbinsX());
    TMatrixDSym C_sum_reint = rarexsec::syst::sum_covariance_block_same_binning(C_reint_block, H_A->GetNbinsX(), H_B->GetNbinsX());
    TMatrixDSym C_sum_pot = rarexsec::syst::sum_covariance_block_same_binning(C_pot_block, H_A->GetNbinsX(), H_B->GetNbinsX());
    ...
    if (H_data)
        leg1.AddEntry(H_data.get(), "Data", "lep");
    leg1.Draw();
