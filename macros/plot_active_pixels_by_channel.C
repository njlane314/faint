...
void plot_active_pixels_by_channel(const char* extra_libs = "",
    ...
    if (mc.empty())
        throw std::runtime_error("No MC samples found");
    ...
    for (char c : plane_key)
        plane_upper.push_back(up(c));

    const bool has_xmax_override = xmax > 0.0;
    const double xmax_override = xmax;
    const double NPIX_plane = 512.0 * 512.0;
    ...
    for (const auto& cfg : plot_configs) {
        ...
        rarexsec::plot::UnstackedHist plot(spec, opt, mc, {}, false, 3);
