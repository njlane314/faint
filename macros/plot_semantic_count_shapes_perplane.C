...
static double sem_count(const std::vector<int>& counts, int lab) {
    if (counts.empty() || lab < 0 || lab >= (int)counts.size())
        return 0.0;
    return static_cast<double>(counts[lab]);
}
static void normalize_pdf(TH1D& h) {
    double A = h.Integral("width");
    if (A > 0.0)
        h.Scale(1.0 / A);
}
...
void plot_semantic_count_shapes_perplane(const char* extra_libs = "",
    ...
    const char* Ycnt = use_event_counts ? "event_semantic_counts_w" : "slice_semantic_counts_w";
    ...
    for (const auto& p : planes) {
        ...
        for (size_t ie = 0; ie < mc.size(); ++ie) {
            ...
            if (!e)
                continue;
            auto n0 = rarexsec::rarexsec::selection::apply(e->rnode(), rarexsec::rarexsec::selection::Preset::Empty, *e);
